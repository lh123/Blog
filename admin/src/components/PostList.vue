    ...
    <div class="list-wrapper">
        <div class="post-list">
            <div class="post-list-head">
                ...
                <span>文章列表</span>
                <img src="../assets/img/add.png" @click="addPost" />
            </div>
            <ul>
                <li v-for="item in draftList" :key="item._id" @click="onDraftClick(item._id)" class="post-list-item" :class="{'post-select':item._id === currentDraftId}">
                    <h3 class="post-title">{{item.title}}</h3>
                    <p class="post-time">{{getFormatDate(item)}}</p>
        ...
        <div class="post-editor-container">
            <transition name="fade" mode="out-in">
                <router-view @onpostSave="fetchData" @onDraftDelete="fetchData" @onDraftSave="fetchData"></router-view>
            </transition>
        </div>
    </div>
</template>
